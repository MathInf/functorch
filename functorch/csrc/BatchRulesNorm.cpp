...
std::tuple<at::Tensor,optional<int64_t>> native_batch_norm_backward_no_weight_bias_batch_rule(
  ...
  if (!grad_out_bdim.has_value() && !input_bdim.has_value() && !running_mean_bdim.has_value() && !running_var_bdim.has_value()) {
    ...
    const auto result = at::native_batch_norm_backward(
        grad_out, input, nullopt, running_mean_opt, running_var_opt, mean, rstd, training, eps, {true, false, false});
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> native_batch_norm_backward_plumbing(
  ...
  if (output_mask[0]) {
    const auto weight_padded = padRight(weight, nullopt, grad_out.dim());            // [C] -> [C, 1*]
    const auto grad_normalized_input = weight.defined() ?
      grad_out.transpose(0, 1) * weight_padded : grad_out.transpose(0, 1);           // [B0, C, B, *]
    ...
    const auto results = native_batch_norm_backward_no_weight_bias_batch_rule(
...
TORCH_LIBRARY_IMPL(aten, FT_BATCHED_KEY, m) {
  ...
  m.impl("native_batch_norm_backward", native_batch_norm_backward_plumbing);
